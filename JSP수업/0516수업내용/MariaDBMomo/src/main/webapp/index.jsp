...
<script type="text/javascript">
   ...
   function updateComment(idx, name, content) {	// 수정하기 누르면 작동
	   $("#cancelBtn").css('display', 'inline');	// 취소버튼 보이고
   	  ...
   	  $("#idx").val(idx);	// 글번호 넣고
   	  $("#name").val(name);		// 이름 넣고
   	  $("#content").val(content);	//내용 넣고
   	  $("#submitBtn").val("수정");		// 버튼의 이름을 수정으로 바꾼다.
   	  $("#mode").val("update");	// 모드를 update로 변경
   	  $("#password").focus();	// 비번 입력자리에 커서 보내기
   ...
   function deleteComment(idx, name, content) { // 삭제하기 누르면 작동
	   $("#cancelBtn").css('display', 'inline');
   	  ...
   	  $("#name").val(name);		// 이름 넣고
   	  $("#content").val(content);	//내용 넣고
   	  $("#submitBtn").val("삭제");		// 버튼의 이름을 삭제로 바꾼다.
   ...
   function resetComment(idx, name, content) { // 취소하기 누르면 작동
	  ...
	  $("#idx").val("0");	// 글번호 넣고
   	  $("#name").val("");		// 이름 넣고
   	  $("#content").val("");	//내용 넣고
   	  $("#submitBtn").val("저장");		// 버튼의 이름을 저장으로 바꾼다.
   	  $("#mode").val("insert");	// 모드를 insert로 변경
   	  $("#name").focus();	// 이름 입력자리에 커서 보내기
   }
</script>
<style type="text/css">
	...
	<table>
		...
		%>
			<tr align="center">
				...
				out.println(content);
				%>
				<input type="button" onclick="updateComment(<%=vo.getIdx() %>,'<%=vo.getName() %>','<%=vo.getContent() %>')" value="수정" class="btn btn-outline-success btn-sm" required="required" />
				<input type="button" onclick="deleteComment(<%=vo.getIdx() %>,'<%=vo.getName() %>','<%=vo.getContent() %>')" value="삭제" class="btn btn-outline-success btn-sm" required="required" />
		...
		<tr>
			<td colspan="5" style="border: none;" align="center">
				...
				<form action="updateOk.jsp" method="post">
					...
					<input type="submit" id="submitBtn" value="저장" class="btn btn-outline-success btn-sm" required="required" />
					<input type="button" id="cancelBtn" value="취소" class="btn btn-outline-success btn-sm" required="required" onclick="resetComment()" />
				</form>
			</td>
		</tr>
