...
public class PagingVO<T> { // 어떤 상황에서도 사용할수 있게 제네릭을 사용
	...
	private void calc() {
		if (totalCount > 0) {
			...
			// 총 123개 가정, 페이지당 : 10개 ==> 13페이지
			// (123-1)/10 + 1 = 13
			// 총 120개 가정, 페이지당 : 10개 ==> 12페이지
			// (120-1)/10 + 1 = 12

			totalPage = (totalCount - 1) / pageSize + 1;

			// 시작 글번호 = (현재페이지 -1) * 페이지당 글 수
			// 오라클은 +1을 해준다. 인덱스가 1부터 이다.
			// 67 개 10씩
			// 1페이지(0~9) = (1-1) * 10 = 0
			...
			// 끝 글번호는 전체 개수를 넘을수 없다. (오라클의 경우는 조건을 >로)
			if (endNo >= totalCount) {
				endNo = totalCount - 1; // 오라클의 경우는 -1을 하지 않는다.
			}

			// 시작페이지 번호 = (현재페이지 - 1) / 페이지 목록개수 * 페이지 목록개수 + 1;
			// 현재 7페이지를 본다면 1 ~ 10 페이지 보이고
			// 현재 13페이지를 본다면 11 ~ 20 페이지가 보여야 한다.
			startPage = (currentPage - 1) / blockSize * blockSize + 1;

			// 끝 페이지 = 시작페이지 + 페이지 목록개수 -1;
			endPage = startPage + blockSize - 1;
			// 끝 페이지는 전체 페이지를 넘을 수 없다.
	...
	public String getPageList() {
		...
		// 마지막 페이지가 전체페이지 수 보다 적을 경우에만 다음이 있다. 
		if(endPage < totalPage) {
			pageList += "	<li class='page-item'>";
