...
<link
	href="${pageContext.request.servletContext.contextPath }/webjars/bootstrap/5.1.3/css/bootstrap.min.css"
	rel="stylesheet">
<script type="text/javascript"
	src="${pageContext.request.servletContext.contextPath }/webjars/jquery/3.6.0/jquery.min.js"></script>
<script type="text/javascript"
	src="${pageContext.request.servletContext.contextPath }/webjars/bootstrap/5.1.3/js/bootstrap.min.js"></script>
<script type="text/javascript">
	$(function() {
...
<body>
	<table>
		...
		<tr>
			<td colspan="4" style="text-align: right; border: none;"><c:url
					var="url" value="updateForm.jsp">
				...
				<button onclick="location.href='${url}'">수정하기</button> <c:url
					var="url" value="deleteForm.jsp">
					<c:param name="p" value="${pv.currentPage }" />
					<c:param name="s" value="${pv.pageSize }" />
					<c:param name="b" value="${pv.blockSize }" />
					<c:param name="idx" value="${vo.idx }" />
				...
				<button onclick="location.href='${url}'">삭제하기</button> <c:url
					var="url" value="index.jsp">
					<c:param name="p" value="${pv.currentPage }" />
					<c:param name="s" value="${pv.pageSize }" />
					<c:param name="b" value="${pv.blockSize }" />
				</c:url>
				<button onclick="location.href='${url}'">돌아가기</button></td>
		</tr>
			<%-- 댓글 폼을 달자 --%>	
		<tr>
			<td colspan="4" style="border: none; text-align: left; ">
				<form action="commentUpdate.jsp" method="post" onsubmit="return cformCheck()">
					...
					<br />
					<textarea name="content" id="content" cols="120" rows="5" placeholder="내용입력" required="required"></textarea>
					<br />				
				...
				<c:forEach var="cvo" items="${vo.commentList}">
					<div style="padding: 5px; border: 1px solid gray;  margin-bottom: 5px;">
						...
						<span id="name${cvo.idx }">${cvo.name }</span>님이 ${cvo.ip }에서
							<fmt:formatDate value="${cvo.regDate }" pattern="yy-MM-dd hh:mm:ss"/>에 남긴 글
							<%-- 수정/삭제 버튼 달기 --%>
	...
	<script type="text/javascript">
		function updateForm(idx) {
			...
			$("#resetBtn").css('display', 'inline');	// 취소 버튼 보이고
			$("#submitBtn").val('수정');
			$("#idx").val(idx);
			$("#mode").val("update");
			$("#name").val(name);
		...
		function resetForm(idx) {
			...
			$("#resetBtn").css('display', 'none');	// 취소 버튼 보이고
			$("#submitBtn").val('저장');
			$("#idx").val('0');
			$("#mode").val("insert");
			$("#name").val("");
