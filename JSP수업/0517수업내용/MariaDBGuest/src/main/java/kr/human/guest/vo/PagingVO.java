...
public class PagingVO<T> {	// T 는 Type
	private List<T> list; // 글 목록을 저장할 변 수
	...
	private int blockSize; 		// 페이지 번호 개수 : 넘겨받는다.
	
	
	// 나머지 값은 계산 한다.
	private int totalPage;		// 전체 페이지 수
	private int startNo;		// 시작 글번호 
	private int endNo; 			// 끝 글번호 (오라클에서만 사용한다.)
	private int startPage;		// 페이지목록 시작 번호
	private int endPage;		// 페이지목록 끝 번호
	...
	private void calc() {
		...
		if(totalCount>0) {	// 데이터가 있을때만 계산
			...
			// 전체 페이지 = (전체 글 갯수 -1) / 페이지당 글 수 + 1
			totalPage = (totalCount -1)/pageSize + 1;
			
			// 시작번호 = (현재페이지 - 1) * 페이지당 글 수 
			...
			// 시작 페이지 번호 = (현재페이지-1)/페이지수 * 페이지수 + 1
			// 3페이지 ==> (3-1)/5 * 5 + 1 = 6
			// 7페이지 ==> (7-1)/5 * 5 + 1 = 6
			startPage = (currentPage-1) / blockSize * blockSize +1;
			
			// 마지막 페이지 번호 = 시작 페이지 + 페이지 수  -1;
	...
	// 계산한 변수들은 변경 되면 안되기 때문에 Getter만 만들고
	// 글 목록은 아직 저장되지 않았으므로 Getter와 Setter를 모두 만든다.
