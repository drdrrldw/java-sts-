...
public class WinEx {
	...
	public static void main(String[] args) {
		...
		// 윈도우가 닫힐때 프로그램도 종료하라고 지정
		// WinClose close = new WinClose();
		// frame.addWindowListener(new WinClose());   // 위의 두줄을 1줄로
		
		// 내부 클래스로 만드는 것이 관리상 편리하다. 하지만 1회성 용도이다.
		// 그렇다면 익명 내부 클래스로 만들어서 쓰면 더 좋지않을까?
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(frame, "왜 눌러~!!");
				
			}
		});
		
		
		frame.add(button);	// 버튼을 윈도우에 붙인것
